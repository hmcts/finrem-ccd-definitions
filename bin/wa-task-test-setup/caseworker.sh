...
if [[ -z "${USERS_FILE}" || -z "${ENVIRONMENT}" || -z "${CLIENT_SECRET}" || -z "${ACTION}" ]]; then
  echo "Usage: ./idam_batch.sh <env> <client_secret> <add|update>"
  exit 1
fi
...
get_access_token() {
  curl -s -X POST \
    ...
    -d "scope=profile roles" | jq -r '.access_token'
}
