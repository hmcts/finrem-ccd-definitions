...
async function historyTab(caseType, tabName, eventName, endState) {
  ...
  //pause();
   ...
   // if (!I.see(tabName)) {
        //await I.waitForNavigationToComplete('//mat-tab-header/div[1]/div[1]');
   // }
 ...
 async function applicantTab(caseType, tabName) {
  ...
  case 'consented':
      ...
      await I.waitForText(verifyTabText.applicantTab.applicantsLocalCourt);
  ...
  case 'contested':
     ...
     await I.see(verifyContestedTabText.applicantTab.applicantLastName);
      await I.waitForText(verifyContestedTabText.applicantTab.applicantSolicitorName);
 ...
 async function respondentTab(caseType, tabName) {
  ...
  case 'contested':
     ...
     await I.waitForText(verifyContestedTabText.respondentTab.respondentPostCode)
...
async function caseDocumentsTab(caseType, tabName, eventName) {
  ...
  case 'contested':
    ...
    if (eventName === verifyContestedTabText.historyTab.assignToJudgeEvent ||
     ...
     await I.waitForText(verifyContestedTabText.caseDocumentsTab.draftOnlineFormA);
    ...
    if (eventName === verifyContestedTabText.historyTab.createGeneralApplicationEvent) {
        await I.waitForText(verifyContestedTabText.caseDocumentsTab.gaApplicationReceivedFrom);
