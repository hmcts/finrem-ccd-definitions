...
import { vacateHearingNotRelistedTableData, vacateHearingRelistedExpressFdrTableData, vacateHearingRelistedStandardFdaTableData, vacateHearingRelistedTableData } from '../../../resources/check_your_answer_content/manage_hearings/manageHearingVacateHearingTable.ts';
...
test.describe('Contested - Vacate Hearings', { tag: ['@MH'] }, () => {
  ...
  test('Contested - Vacate Hearing - Relisted - Standard FDA Hearing',
    { tag: [] }, async ({ loginPage, manageCaseDashboardPage, caseDetailsPage, manageHearingPage, axeUtils, checkYourAnswersPage }) => {
      // Create and setup case up to issue application
      const caseId = await ContestedCaseFactory.progressToUploadDraftOrder({ isFormA: true });
      await ContestedEventApi.caseworkerAddsApplicantIntervener(caseId);
      ...
      //check your answers page
      await checkYourAnswersPage.assertCheckYourAnswersPage(vacateHearingRelistedStandardFdaTableData);
      await manageHearingPage.navigateSubmit();
      ...
      await caseDetailsPage.assertTabData([relistedHearingData]);

      await manageCaseDashboardPage.signOut();
    }
  );
  test('Contested - Vacate Hearing - Relisted - Express FDR Hearing',
    { tag: [] }, async ({ loginPage, manageCaseDashboardPage, caseDetailsPage, manageHearingPage, axeUtils, checkYourAnswersPage }) => {
      // Create and setup case up to issue application
      ...
      //check your answers page
      await checkYourAnswersPage.assertCheckYourAnswersPage(vacateHearingRelistedExpressFdrTableData);
      await manageHearingPage.navigateSubmit();
