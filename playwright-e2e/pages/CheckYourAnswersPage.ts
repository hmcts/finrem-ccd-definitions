...
export class CheckYourAnswersPage {
  ...
  async assertCheckYourAnswersPage(table: Table) {
    ...
    const normalize = (val: string) => {return val.replace(/[\n\t]/g, '').trim();};
    ...
    for (const expected of table.rows) {
      if (typeof expected === 'string') {
        ...
        const found = actualRows
          .some(([cellItem, cellValue]) => {return cellItem === expectedNorm || cellValue === expectedNorm;});
        if (!found) {
        ...
        const found = actualRows.some(
          ([cellItem, cellValue]) => {return cellItem === cellItemNorm && cellValue === valueNorm;}
        );
        if (!found) {
