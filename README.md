...
in the -wanonprod.json version of the file and make any wa specific changes in the -wanonprod.json file. Also include the base event in nonprod.json file so to maintain the base functionality for non prod environments. Also copy to the -prod.json only if your event has prod specific behaviour.
